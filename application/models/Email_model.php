...
class Email_model extends CI_Model
{
	...
	public function suspended_offline_payment($user_id = "")
	{
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			...
			//Editable
			if($user_type == 'student'){
				$to_user = $this->db->get_where('users', array('id' => $user_id))->row_array();
			}
			//End editable
	...
	public function send_email_when_approed_an_instructor($id)
	{
		//Editable
		$type = 'instructeur_approval_notification';
		//End editable
		$to_user = $this->db->get_where('users', array('id' => $id))->row_array();

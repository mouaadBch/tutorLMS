...
class Payment_model extends CI_Model {
    ...
    function checkLogin($payment_info = "",$type=null){
      if($this->session->userdata('user_id') > 0)
      {
          ...
          $cart_items = array();
          /* code mouaad */
          if (mb_strlen($payment_info, 'UTF-8') < 25) {
            $data = $this->db->select("*")
              ->where('oid', $payment_info)
              ->get('cmi_payment')->row();
            $payment_info = $data->cmiSessionData;
          }
          ...
          if ($type=='bundle' || $type=='ebook' || $type=='booking') {

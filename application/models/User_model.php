...
class User_model extends CI_Model
{
    ...
    /* code mouaad */
    function get_user_by_course_enrol($user_id)
    {
        $courses = $this->db->select('id')->where('creator', $user_id)->get('course')->result_array();
        if(!empty($courses)){
            $this->db->where_in('course_id',  array_column($courses, 'id'));
    ...
    { // Admin does this editing
        ...
        if ($validity) {
            ...
            if(strtolower($this->session->userdata('role')) == 'admin'){
                $data['percentage_rev_cours'] = $this->input->post('instructor_revenue_coures');
                $data['percentage_rev_ebook'] = $this->input->post('instructor_revenue_ebook');
                $data['percentage_rev_tutor'] = $this->input->post('instructor_revenue_titoring');
            }
            /* code mouaad */
    ...
    public function post_instructor_application($user_id = "")
    {
        ...
        if ($user_details['email'] == $email) {
            ...
            //$this->db->update('users',array('phone'=>$this->input->post('phone')));
            if (isset($_FILES['document']) && $_FILES['document']['name'] != "") {
                if (!file_exists('uploads/document')) {
                    mkdir('uploads/document', 0777, true);
                }
                $accepted_ext = array('docx','doc', 'docs', 'pdf', 'txt', 'png', 'jpg', 'jpeg');
                $path = $_FILES['document']['name'];
                $ext = pathinfo($path, PATHINFO_EXTENSION);
                if (in_array(strtolower($ext), $accepted_ext)) {
                    $document_custom_name = $user_details['last_name'].'_' .$user_details['first_name'] . '_' . $user_id .'__' . random(5) . '.' . $ext;
    ...
    function instructor_application(){
        ...
        if($user->num_rows() > 0){
            ...
            if ($previous_data == 0) {
                ...
                $data['phone'] = $this->input->post('phone');
                $data['message'] = $this->input->post('message');
                $this->db->where('id' , $user_details['id'])->update('users', array('phone'=> $this->input->post('phone') ));

                $document_custom_name =$user_details['last_name'].'_' .$user_details['first_name'] . '_' . $user_details['id'] .'__' . random(5) .'.'.pathinfo($_FILES['document']['name'], PATHINFO_EXTENSION);
                $data['document'] = $document_custom_name;
                move_uploaded_file($_FILES['document']['tmp_name'], 'uploads/document/' . $document_custom_name);
                $data['created_at'] = time();
                $this->db->insert('applications', $data);
    ...
    {
            $applications = $this->db->order_by("id", "DESC")->get_where('applications', array('status' => 1));
        return $applications;
    }
    ...
    {
        $applications = $this->db->order_by("id", "DESC")->get_where('applications', array('status' => 0));
        return $applications;
    }
    ...
    public function update_status_of_application($status, $application_id)
    {
        ...
        if ($application_details->num_rows() > 0) {
            ...
            if ($status == 'approve') {
                ...
                /* code mouaad */
                $this->crud_model->enrol_to_course_user(69, $application_details['user_id']);
                $this->email_model->send_email_when_approed_an_instructor($application_details['user_id']);
                /* code mouaad */
                ...
                $application_data['status'] = 0;
                $this->db->where('id', $application_id);
                $this->db->update('applications', $application_data);

                $instructor_data['is_instructor'] = 0;
                $this->db->where('id', $application_details['user_id']);
                $this->db->update('users', $instructor_data);
    ...
    function set_login_userdata($user_id = ""){
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                }elseif(($this->db->where('user_id',$user_id)->where_not_in('course_id', [71,69])->get("enrol")->num_rows()) > 0){

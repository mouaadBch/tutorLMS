...
//v5.7
require_once APPPATH . 'libraries/BunnyVideo/BunnyCDNStream.php';
...
class Crud_model extends CI_Model
{
    ...
    //code mouaad
    public function enrol_to_course_user($course_id, $user_id)
    {
        ...
        if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
            ...
            $course_details = $this->db->select('expiry_period')->get_where('course', array('id' => $data['course_id']))->row_array();
            $data['expiry_date'] = $course_details['expiry_period'] * 30 * 86400 + $today;
    ...
    public function get_revenue_by_user_type($timestamp_start = "", $timestamp_end = "", $revenue_type = "", $selected_instructor_id = 'all')
    {
        $course_ids = array();
        $courses    = array();
        ...
        if ($revenue_type == 'admin_revenue') {
            ...
            $this->db->where('date_added >=', $timestamp_start);
            $this->db->where('date_added <=', $timestamp_end);
            if ($selected_instructor_id != 'all') {
                $this->db->where('user_id =', $selected_instructor_id);
            }
            /* code mouaad */
    ...
    public function purchase_history($user_id = "", $timestamp_start = "", $timestamp_end = "")
    {
        if (isset($timestamp_start)) {
            $this->db->where('date_added >=', $timestamp_start);
            $this->db->where('date_added <=', $timestamp_end);
        }
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        $data['is_affiliate'] = ($this->input->post('is_affiliate') == 1) ? 1 : 0;
        if ($this->session->userdata('admin_login')) {
            $data['orderBy'] = $this->input->post('course_order');
        }
        $enable_drip_content = $this->input->post('enable_drip_content');
        if (isset($enable_drip_content) && $enable_drip_content) {
            $data['enable_drip_content'] = 1;
    ...
    public function add_lesson()
    {
        if (empty($this->input->post('title'))) {
            return json_encode(['error' => "Title is required. Please provide a valid title."]);
        }
        ...
        if ($lesson_type == 'video') {
            ...
            if (isset($_FILES['video_file_for_amazon_s3'])) {
                ...
                $video_url = $this->input->post('video_link_for_amazon_s3');
            }
            ...
            $libId = '171846';
            $ApiAccess = 'bf492b0f-d95a-4677-a7831ef8168c-32ca-468a';

            if (isset($_FILES['video_file_for_bunny'])) {
                $bunny = new BunnyVideo\BunnyCDNStream($libId, $ApiAccess);
                $tmpfile = $_FILES['video_file_for_bunny']['tmp_name'];
                try {
                    $title = preg_replace('/[^A-Za-z0-9 ]/', '', $data['title']);
                    // Optionally, you can trim any leading or trailing spaces
                    $title = trim($title);

                    $videoObject = $bunny->createVideo($title);
                    $uploadeVideo = $bunny->uploadVideoWithVideoId($videoObject["guid"], $tmpfile);
                    $getVideo = $bunny->getVideo($videoObject["guid"]);
                } catch (Exception $e) {
                    throw $e;
                }
                if ($uploadeVideo['success'] != true) {
                    return json_encode(['error' => get_phrase('please_select_valid_video_file')]);
                }
                $video_url = "https://iframe.mediadelivery.net/embed/" . $libId . "/" . $getVideo['guid'];
            } else {
                $video_url = $this->input->post('video_link_for_bunny');
            }
            ...
            $duration_formatter = explode(':', $this->input->post('bunny_duration'));
            $hour = sprintf('%02d', $duration_formatter[0]);
            $min = sprintf('%02d', $duration_formatter[1]);
            $sec = sprintf('%02d', $duration_formatter[2]);
            $data['duration'] = $hour . ':' . $min . ':' . $sec;
    ...
    public function edit_lesson($lesson_id)
    {
        ...
        if ($lesson_type == 'video') {
            ...
            if (isset($_FILES['video_file_for_bunny']) && !empty($_FILES['video_file_for_bunny']['name'])) {
                $tmpfile = $_FILES['video_file_for_bunny']['tmp_name'];
                try {
                    $title = preg_replace('/[^A-Za-z0-9 ]/', '', $data['title']);
                    $title = trim($title);
            ...
            $oldParts = explode('/', $previous_data['video_url']);
            $newParts = explode('/', $video_url);

            if ($newParts[5] != $oldParts[5]) {
                $bunnyVideoDelete = $bunny->deleteVideo($oldParts[5]);
            }
    ...
    public function delete_lesson($lesson_id)
    {
        ...
        if ($previous_data['video_type'] == 'bunny') {
            $Parts = explode('/', $previous_data['video_url']);
            $libId = '171846';
            $ApiAccess = 'bf492b0f-d95a-4677-a7831ef8168c-32ca-468a';
            $bunny = new BunnyVideo\BunnyCDNStream($libId, $ApiAccess);
            $deletevideo = $bunny->deleteVideo($Parts[5]);
        }
    ...
    {
        if ($payer_user_id == 'bundle') {
            /* $bundle = $this->db->where('id', $id)->get('offline_payment')->row();
            $data['user_id'] = $enrol_user_id;
            $data['gifted_by'] = 0;
            $data['course_id'] = json_decode($bundle->course_id)[0];
            $data['date_added'] = strtotime(date('D, d-M-Y'));
            $this->db->insert('enrol', $data); */
        } elseif ($payer_user_id == 'ebook') {
            /* $ebook = $this->db->where('id', $id)->get('offline_payment')->row();
            $data['user_id'] = $enrol_user_id;
            $data['gifted_by'] = 0;
            $data['course_id'] = json_decode($ebook->course_id)[0];
            $data['date_added'] = strtotime(date('D, d-M-Y'));
            $this->db->insert('enrol', $data); */
        } else {
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "")
    {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if ($method == 'cmi') {
                $cmi_payment = $this->db->where('oid', $_GET['i'])->get('cmi_payment')->row_array();
                $this->session->set_userdata('course_referee', $cmi_payment['course_referee']);
                $this->session->set_userdata('course_reffer_id', $cmi_payment['course_reffer_id']);
            }
            ...
            if (get_user_role('role_id', $course_details['creator']) == 1) {
                ...
                if (get_settings('allow_instructor') == 1) {
                    ...
                    /* code mouaad */
                    $user = $this->user_model->get_all_user($course_details['creator'])->row_array();
                    $instructor_revenue_percentage = $user['percentage_rev_cours'] ?? get_settings('instructor_revenue');
                    /* code mouaad */
            ...
            if (addon_status('affiliate_course')) :
                if ($this->session->userdata('course_referee') != "" && $this->session->userdata('course_reffer_id')) {
                    ...
                    $course_affiliation['payment_id'] = $data['transaction_id'] ?? null;
    ...
    function filter_course($search_string = "", $selected_category_id = "", $selected_price = "", $selected_level = "", $selected_language = "", $selected_rating = "", $selected_sorting = "", $per_page = "", $uri_segment = "")
    {
        ...
        if ($selected_sorting != "" && $selected_sorting == 'newest') {
            ...
            $this->db->order_by('avg_rating', 'desc');
        } elseif ($selected_sorting != "" && $selected_sorting == 'affiliate') {
            $this->db->order_by('is_affiliate', 'desc');
        }
    ...
    public function get_courses($category_id = "", $sub_category_id = "", $instructor_id = 0, $status = "")
    {
    ...
    function manage_mcq_choice_question($quiz_id, $question_id, $action)
    {
        ...
        /* code Mouaad */
        $data['justify']            = htmlspecialchars_($this->input->post('justify', false));
        /* code Mouaad */
        if ($action == 'add') {
    ...
    function manage_fill_in_the_blank_question($quiz_id, $question_id, $action)
    {
        ...
        /* code mouaad */
        $x = array();
        foreach (json_decode($this->input->post('correct_answers'), true) as $i => $correct_answer) :
            $x[] = ($correct_answer['value']);
        endforeach;
        $data['correct_answers']    = json_encode($x);
        /* / code mouaad */
    ...
    function get_course_by_user($user_id = "", $course_type = "")
    {
        ...
        $this->db->group_start();
        $this->db->like('user_id', ',' . $user_id); //instructor_id
        $this->db->or_like('user_id', $user_id . ','); //instructor_id
        $this->db->or_where('creator', $user_id); //instructor_id
        $this->db->group_end();
    ...
    function update_website_faq()
    {
        ...
        foreach ($languages as $language) :
            if (empty($this->input->post('questions')[$language])) {
                $faqs[$language][0]['question'] = '';
                $faqs[$language][0]['answer'] = '';
            } else {
                foreach ($this->input->post('questions')[$language] as $key => $question) :
                    $faqs[$language][$key]['question'] = $question ?? '';
                    $faqs[$language][$key]['answer'] = $this->input->post('answers')[$language][$key] ?? '';
                endforeach;
            }
        endforeach;
    ...
    function update_motivational_speech()
    {
        ...
        $languages = $this->crud_model->get_all_languages();
        foreach ($languages as $language) :
            if (empty($this->input->post('titles')[$language])) {
                $motivations[$language][0]['status'] = '';
                ...
                foreach ($this->input->post('titles')[$language] as $key => $title) {
                    ...
                    if ($_FILES['images']['name'][$language][$key] != "") {
                        $image_name = random(20) . '.png';
                        move_uploaded_file($_FILES['images']['tmp_name'][$language][$key], 'uploads/system/motivations/' . $image_name);
                        $motivations[$language][$key]['image'] = $image_name;
                    } else {
                        $motivations[$language][$key]['image'] = $this->input->post('previous_images')[$language][$key] ?? '';
                    }
                    $images[$i] = $motivations[$language][$key]['image'];
                    $i++;
                }
            }
        endforeach;
        ...
        $data['value'] = json_encode($motivations, JSON_UNESCAPED_UNICODE);

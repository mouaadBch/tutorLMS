                                                ...
                                                <div class="conversation-1">
                                                    <div class="conversation-heading">
                                                        <div class="conversation-img">
                                                            <img class="rounded-circle" src="<?php echo $this->user_model->get_user_image_url($user_to_show_id);?>">
                                ...
                                <div class="conversation-text message-box-content">
                                    <div class="conversation-text-2" style="overflow: auto;">
                                        <div class="conversation-msg-full">
                                            <div class="parrent">
                                                <div class="child-img">
                                                    <img src="<?php echo $this->user_model->get_user_image_url($user_to_show_id);?>" class="parrent-image mb-2">
                                                </div>
                                    ...
                                    <div class="conversation-sending">
                                        <form action="<?php echo site_url('home/my_messages/send_reply/'.$message_thread_code); ?>" method="post">
                                            ...
                                            <div class="search-btn"> 
                                                <label for="img">
                                                    <img src="<?php echo base_url('assets/frontend/default-new/image/msg-paper.png') ?>">
                                                </label>
                                    ...
                                    <form action="<?php echo site_url('home/my_messages/send_new'); ?>" method="post" class="mt-5">
                                        <div class="form-group mb-3">
                                            <?php 
                                                ...
                                                $instructor_list2 = array();
                                                foreach ($my_courses as $my_course) {
                                                    $course_details = $this->crud_model->get_course_by_id($my_course['course_id'])->row_array();
                                                    if(isset($course_details['creator'])){
                                                        $infoInstructor = $this->user_model->get_all_user($course_details['creator'])->row_array();
                                                        $data = [
                                                            'id' => $course_details['creator'],
                                                            'first_name' => $infoInstructor['first_name'],
                                                            'last_name' => $infoInstructor['last_name'],
                                                        ];
                                                        array_push($instructor_list2, $data);
                                                    }
                                                }

                                                if ($this->session->userdata('is_instructor') == 1) {
                                                    $users_by_course_enrol = $this->user_model->get_user_by_course_enrol($this->session->userdata('user_id'));
                                                    if ($users_by_course_enrol) {
                                                        foreach ($users_by_course_enrol as $userCourse) {
                                                            $user = $this->user_model->get_all_user($userCourse['user_id'])->row_array();
                                                            $data = [
                                                                'id' => $userCourse['user_id'],
                                                ...
                                                $instructor_list2 = array_values(array_column($instructor_list2, null, 'id')); 
                                            ?>
                                            <label><?php echo get_phrase('Select a user'); ?></label>
                                            <select class="form-control my-2 py-3 radius-5" name = "receiver">
                                                <?php foreach ($instructor_list2 as $instructor):

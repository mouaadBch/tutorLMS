...
$this->session->set_userdata('total_price_of_checking_out',$booking_info['price']);
$this->session->set_userdata('booking_info',$booking_info['id']);
$this->session->set_userdata('schedule_info',$schedule_info['id']);
$booking_amount=$amount;
$this->session->set_userdata('booking_amount',$booking_amount);
$this->session->set_userdata('total_price_of_checking_out',$booking_amount);
		...
		<div class="col-md-8">
			<div class="row">
				<div class="col-md-12">
					...
					<a href="<?php echo site_url('schedules_bookings/'.$this->session->userdata('tutor_id')); ?>" class="close-btn-light float-end"><i class="fa fa-times"></i></a>
				</div>
			</div>
		</div>
	</div>
				...
				<div class="col-md-8">
					<div class="w-100 d-grid">
						...
						<?php 
							...
							$instructor_details = $this->user_model->get_all_user($schedule_info['tutor_id'])->row_array(); ?>

							<p class="item float-start">
								<span class="count-item"><?php echo $counter; ?></span>
								<span class="item-title"><?php echo $booking_info['title']; ?>
									<span class="item-price">
										<?php 
											echo currency($booking_amount);

											echo "  (".get_phrase($booking_info['price_type']).")";
									 ?>
									</span>
								</span>
								<span class="by-owner">
					...
					<div class="w-100 float-start mt-4 indicated-price">
						<div class="float-end total-price"><?php echo currency($booking_amount); ?></div>
						<div class="float-end total"><?php echo get_phrase('total'); ?></div>
					</div>
					<div class="w-100 float-start">
						<form action="<?php echo site_url('addons/tutor_booking/paypal_checkout'); ?>" method="post" class="paypal-form form">
							<hr class="border mb-4">
							<input type="hidden" name="total_price_of_checking_out" value="<?php echo $booking_info['price']; ?>">
							<input type="hidden" name="purchesed_booking_id" value="<?php echo $booking_info['id']; ?>">
							<input type="hidden" name="purchesed_schedule_id" value="<?php echo $schedule_info['id']; ?>">
						...
						<form action="<?php echo site_url('addons/tutor_booking/razorpay_checkout'); ?>" method="post" class="razorpay-form form">
							<hr class="border mb-4">
							<input type="hidden" name="total_price_of_checking_out" value="<?php echo $booking_info['price']; ?>">
							<input type="hidden" name="purchesed_booking_id" value="<?php echo $booking_info['id']; ?>">
							<input type="hidden" name="purchesed_schedule_id" value="<?php echo $schedule_info['id']; ?>">

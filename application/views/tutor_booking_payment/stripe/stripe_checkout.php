...
$purchesed_schedule_id= $this->session->userdata('schedule_info');
$purchesed_booking_id=$this->session->userdata('booking_info');
...
$stripeAmount = round($booking_amount*100, 2);
...
define('STRIPE_SUCCESS_URL', site_url('addons/tutor_booking/stripe_payment/'.$purchesed_schedule_id.'/'.$purchesed_booking_id.'/'.$user_details['id'].'/'.$booking_amount.'/'.$payment_request));
    ...
    try {
        $session = \Stripe\Checkout\Session::create([
            ...
            'line_items' => [[
                'price_data' => [
                    'product_data' => [
                        'name' => get_phrase('Book').' '.get_phrase('Tution')
                    ],

                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane" id="motivational_speech">
                        ...
                        <form action="<?php echo site_url('admin/frontend_settings/motivational_speech'); ?>" method="post" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-md-12">
                                        <?php $motivational_speeches = count(json_decode(get_frontend_settings('motivational_speech'), true)) > 0 ? json_decode(get_frontend_settings('motivational_speech'), true):['ar'=>[['status'=> 0,'title' => '', 'description' => '', 'image' => '']],'fr'=>[['status'=> 0,'title' => '', 'description' => '', 'image' => '']],'en'=>[['status'=> 0,'title' => '', 'description' => '', 'image' => '']]]; ?>
                                                    ...
                                                    <div class="d-flex mt-2">
                                                                    ...
                                                                    <div class="col-md-8">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <div class="custom-file">
                                                                                <input name="previous_images[<?= $language ?>][]" type="hidden" value="<?php echo $motivational_speech['image']; ?>">
                                                                                <input type="file" class="custom-file-input" name="images[<?= $language ?>][]" onchange="changeTitleOfImageUploader(this)" accept="image/*">
                                                        ...
                                                        <?php if($key == 0): ?>
                                                            <div class="" style="padding-top: 32px;">
                                                                <button type="button" class="btn btn-success btn-sm"  name="button" onclick="appendMotivational_speech('<?= $language ?>')"> <i class="fa fa-plus"></i> </button>
                                                            </div>
                                                        <?php else: ?>
                                                            <div class="" style="padding-top: 32px;">
                        ...
                        <?php $languages = $this->crud_model->get_all_languages();
                            foreach ($languages as $language): ?>
                                <div id = "blank_motivational_speech_field_<?= $language ?>">
                                    <div class="d-flex pt-2 border-top">
                                        <div class="flex-grow-1 px-3 row align-items-center">
                                            <div class="form-group  col-md-8">
                                                <label><?php echo get_phrase('Title'); ?></label>
                                                <input type="text" class="form-control" name="titles[<?= $language ?>][]" placeholder="<?php echo get_phrase('faq_question'); ?>">
                                            </div>
                                            <div class="mb-2  col-md-4">
                                                <label class="form-label"><?= site_phrase('status') ?></label>
                                                <select name="status[<?= $language ?>][]" style="display: block;width: 100%;padding: 0.375rem 2.25rem 0.375rem 0.75rem;font-size: 1rem;font-weight: 400;" required>
                                                    <option selected value="1"><?= site_phrase('active')?></option>
                                                    <option value="0"><?= site_phrase('inactive')?></option>
                                                </select>
                                            </div>
                                            <div class="form-group  col-md-8">
                                                <label><?php echo get_phrase('Description'); ?></label>
                                                <textarea name="descriptions[<?= $language ?>][]" class="form-control mt-2" placeholder="<?php echo get_phrase('Description'); ?>"></textarea>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label><?php echo get_phrase('Image'); ?></label>
                                                    <div class="custom-file">
                                                        <input name="previous_images[<?= $language ?>][]" type="hidden" value="">
                                                        <input type="file" class="custom-file-input" name="images[<?= $language ?>][]" onchange="changeTitleOfImageUploader(this)" accept="image/*">
                                                        <label class="custom-file-label" for="addon_zip"><?php echo get_phrase('Upload image'); ?></label>
                                                    </div>
                                                </div>
                    ...
                    <div class="tab-pane" id="websitefaqs">
                        ...
                        <form action="<?php echo site_url('admin/frontend_settings/website_faq'); ?>" method="post" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-md-10">
                                        <?php $faqs = count(json_decode(get_frontend_settings('website_faqs'), true)) > 0 ? json_decode(get_frontend_settings('website_faqs'), true):["ar"=>[['question' => '', 'answer' => '']],"fr"=>[['question' => '', 'answer' => '']],"en"=>[['question' => '', 'answer' => '']]];  
                                        $languages = $this->crud_model->get_all_languages();
                                        foreach ($languages as $language): ?>
                                            <div id = "faq_area_<?= $language ?>" class="my-2 p-2" style="border: 3px solid #<?= rand(100000,999999)?>;">
                                                <h4><?= $language ?></h4>
                                                <?php foreach($faqs[$language] as $key => $faq): ?>
                                                    <div class="d-flex mt-2 ">
                                                        <div class="flex-grow-1 px-3 mb-2">
                                                            <div class="input-group">
                                                                <label class="input-group-text"><?php echo get_phrase('Question'); ?></label>
                                                                <input type="text" class="form-control" name="questions[<?= $language?>][]" id="questions" placeholder="<?php echo get_phrase('faq_question'); ?>" value="<?php echo $faq['question']; ?>">
                                                            </div>
                                                            <div class="input-group mt-1">
                                                                <label class="input-group-text"><?php echo get_phrase('Answer'); ?></label>
                                                                <textarea name="answers[<?= $language?>][]" class="form-control" placeholder="<?php echo get_phrase('answer'); ?>"><?php echo $faq['answer']; ?></textarea>
                                                            </div>
                                                        </div>
                                                        <?php if($key == 0): ?>
                                                            <div class="" style="padding-top: 32px;">
                                                                <button type="button" class="btn btn-success btn-sm"  name="button" onclick="appendFaq('<?= $language?>')"> <i class="fa fa-plus"></i> </button>
                                                            </div>
                                                        <?php else: ?>
                                                            <div class="" style="padding-top: 32px;">
                                                                <button type="button" class="btn btn-danger btn-sm" style="margin-top: 0px;" name="button" onclick="removeFaq(this)"> <i class="fa fa-minus"></i> </button>
                            ...
                            <?php $languages = $this->crud_model->get_all_languages();
                                foreach ($languages as $language): ?>
                                    <div id = "blank_faq_field_<?= $language ?>">
                                        <div class="d-flex pt-2 border-top">
                                            <div class="flex-grow-1 px-3">
                                                <div class="form-group">
                                                    <label><?php echo get_phrase('Question'); ?></label>
                                                    <input type="text" class="form-control" name="questions[<?= $language ?>][]" id="questions" placeholder="<?php echo get_phrase('faq_question'); ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label><?php echo get_phrase('Answer'); ?></label>
                                                    <textarea name="answers[<?= $language ?>][]" class="form-control mt-2" placeholder="<?php echo get_phrase('answer'); ?>"></textarea>
                                                </div>
                                            </div>
                                            <div class="" style="padding-top: 32px;">
                                                <button type="button" class="btn btn-danger btn-sm" style="margin-top: 0px;" name="button" onclick="removeFaq(this)"> <i class="fa fa-minus"></i> </button>
...
<script type="text/javascript">
    <?php $languages = $this->crud_model->get_all_languages();
            foreach ($languages as $language): ?>
                var blank_faq_<?= $language ?> = jQuery('#blank_faq_field_<?= $language ?>').html();
                var blank_motivational_speech_<?= $language ?> = jQuery('#blank_motivational_speech_field_<?= $language ?>').html();
    <?php endforeach;?>
    ...
    $(document).ready(function () {
        ...
        <?php $languages = $this->crud_model->get_all_languages();
            foreach ($languages as $language): ?>
                jQuery('#blank_faq_field_<?= $language ?>').hide();
                jQuery('#blank_motivational_speech_field_<?= $language ?>').hide();
        <?php endforeach;?>
    ...
    function appendFaq(language) {
        var faqa = 'blank_faq_'+language
      jQuery('#faq_area_'+language).append(window[faqa]);
    }
    function removeFaq(faqElem) {
      jQuery(faqElem).parent().parent().remove();
    }

    function appendMotivational_speech(language) {
        var blank_motivational = 'blank_motivational_speech_'+language
      jQuery('#motivational_speech_area_'+language).append(window[blank_motivational]);
    }

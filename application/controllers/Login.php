...
class Login extends CI_Controller
{
    ...
    public function register()
    {
        ...
        $data['first_name'] = $this->str_without_accents($this->input->post('first_name'));
        $data['last_name']  = $this->str_without_accents($this->input->post('last_name'));
        $data['email']  = html_escape($this->input->post('email'));
        $data['phone']  = html_escape($this->input->post('phone'));
        $data['password']  = sha1($this->input->post('password'));

        if (($data['first_name'] !== $this->input->post('first_name')) || ($data['last_name'] !== $this->input->post('last_name'))) {
            $this->session->set_flashdata('error_message', site_phrase('please_remove_any_special_characters_from_your_personal_information_and_try_again'));
